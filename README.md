...
- **Admin**: go to `/admin/`, sign in, create a QR code — pick a title (e.g. "9 July 2026 Meeting"), an optional subtitle, the sub text shown under the QR code, an optional PNG or JPG background image (shown behind the QR code on the display page; images over 5 MB are automatically compressed in the browser before upload), and a theme. Themes include solid/gradient palettes (Classic, Midnight, Sunset, Forest, Paper, Ink, Clay, Sage) and **Social**, which renders the display page as an Instagram-style post — selecting it reveals extra fields for a profile photo, the poster's name, a location, and a likes count. Each event gets its own QR code and email list.
